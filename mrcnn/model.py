...
def load_image_gt(dataset, config, image_id, augmentation=None, use_mini_mask=False):
    ...
    """
    # # Load image and mask
    # if dataset.image_info[image_id]["source"] == "GB06Dataset":
    #     image = dataset(dataset.data_dir, image_id)
    # else:
...
class DataGenerator(KU.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for b, image_id in enumerate(image_ids):
            augmentation = (
                np_random_genrator.choice(self.augmentations)
                if self.augmentations
                else self.augmentations
            )

            image, image_meta, gt_class_ids, gt_boxes, gt_masks = load_image_gt(
                self.dataset, self.config, image_id, augmentation=augmentation
            )
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            gt_boxes = KL.Lambda(lambda x: norm_boxes_graph(x[0], K.shape(x[1])[1:3]))(
        ...
        if mode == "training":
            ...
            class MaskRCNN_TF2(tf.keras.Model):
                ...
                def train_step(self, data):
                    ...
                    with tf.GradientTape() as tape:
                        ...
                        rpn_class_loss = rpn_class_loss_graph(
                            input_rpn_match, rpn_class_logits
                        )
                        rpn_bbox_loss = rpn_bbox_loss_graph(
                            self.config, input_rpn_bbox, input_rpn_match, rpn_bbox
                        )
                        class_loss = mrcnn_class_loss_graph(
                            target_class_ids, mrcnn_class_logits, active_class_ids
                        )
                        bbox_loss = mrcnn_bbox_loss_graph(
                            target_bbox, target_class_ids, mrcnn_bbox
                        )
                        mask_loss = mrcnn_mask_loss_graph(
                        ...
                        combined_loss = tf.math.reduce_sum(
                            [
                                rpn_class_loss,
                                rpn_bbox_loss,
                                class_loss,
                        ...
                        reg_losses = tf.reduce_sum(
                            [
                                tf.keras.regularizers.l2(config.WEIGHT_DECAY)(w)
            ...
            model = MaskRCNN_TF2(
                config=config, inputs=inputs, outputs=outputs, name="mask_rcnn"
            )
    ...
    def train(
        ...
        train_generator = DataGenerator(
            train_dataset, self.config, shuffle=True, augmentations=augmentations
        )
        ...
        self.keras_model.compile(
            optimizer=optimizer, run_eagerly=self.config.RUN_EAGERLY
        )
